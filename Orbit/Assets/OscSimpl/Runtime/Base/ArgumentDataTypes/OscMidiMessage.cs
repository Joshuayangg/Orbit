...
public struct OscMidiMessage
{
	...
	{
		if( sb == null ) sb = new StringBuilder( 18 );
		else sb.Clear();
	...
	{
		return sb.Append( "MIDI(" )
			.AppendGarbageFree( (uint) _port, false ).Append( ',' )
			.AppendGarbageFree( (uint) _status, false ).Append( ',' )
			.AppendGarbageFree( (uint) _data1, false ).Append( ',' )
			.AppendGarbageFree( (uint) _data2, false ).Append( ')' );
	}
